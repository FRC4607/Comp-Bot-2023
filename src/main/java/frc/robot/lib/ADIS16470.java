...
public class ADIS16470 implements AutoCloseable, NTSendable {
    ...
    private static final int X_DELTA_ANG_LOW = 0x24; // X-axis delta angle output, lower word
    private static final int X_DELTA_ANG_OUT = 0x26; // X-axis delta angle output, upper word
    private static final int Y_DELTA_ANG_LOW = 0x28; // Y-axis delta angle output, lower word
    private static final int Y_DELTA_ANG_OUT = 0x2A; // Y-axis delta angle output, upper word
    private static final int Z_DELTA_ANG_LOW = 0x2C; // Z-axis delta angle output, lower word
    private static final int Z_DELTA_ANG_OUT = 0x2E; // Z-axis delta angle output, upper word
    private static final int X_DELTA_VEL_LOW = 0x30; // X-axis delta velocity output, lower word
    private static final int X_DELTA_VEL_OUT = 0x32; // X-axis delta velocity output, upper word
    private static final int Y_DELTA_VEL_LOW = 0x34; // Y-axis delta velocity output, lower word
    private static final int Y_DELTA_VEL_OUT = 0x36; // Y-axis delta velocity output, upper word
    private static final int Z_DELTA_VEL_LOW = 0x38; // Z-axis delta velocity output, lower word
    private static final int Z_DELTA_VEL_OUT = 0x3A; // Z-axis delta velocity output, upper word
    ...
    /**
     * The calibration times available to The gyro.
     */
    ...
    /**
     * The available axis that can be read.
     */
    public enum IMUAxis {
        kX,
        kY,
    ...
    public ADIS16470(IMUAxis yaw_axis, SPI.Port port, CalibrationTime cal_time) {
        m_yaw_axis = yaw_axis;
        m_calibration_time = cal_time.m_value;
        m_spi_port = port;
        ...
        if (m_simDevice == null) {
             ...
             */
             ...
             // Test 1: MSC_CTRL - 0b11000001, NULL_CNFG - 0b0011111100001011, DEC_RATE - 4, FILT_CTRL - 0
             // Test 2: MSC_CTRL - 0b00000001, NULL_CNFG - 0b0011111100001011, DEC_RATE - 4, FILT_CTRL - 0
             // Test 3: MSC_CTRL - 0b00000001, NULL_CNFG - 0b0000011100001011, DEC_RATE - 4, FILT_CTRL - 0
             // Test 4: MSC_CTRL - 0b11000001, NULL_CNFG - 0b0000011100001011, DEC_RATE - 4, FILT_CTRL - 0
             // Test 5: MSC_CTRL - 0b11000001, NULL_CNFG - 0b0000011100001010, DEC_RATE - 4, FILT_CTRL - 0
             // Test 6: MSC_CTRL - 0b11000001, NULL_CNFG - 0b0000011100001011, DEC_RATE - 4, FILT_CTRL - 0
            /* 
             * Linear g compensation for gyroscopes enabled
             * Point of percussion alignment enabled
             * SYNC function setting:  internal clock mode
             * SYNC polarity: falling edge triggers sampling
             * DR polarity: active high when data is valid
            */
            ...
            // DriverStation.reportError( "ADIS16470 MSC_CTRL register: " +
            // readRegister(MSC_CTRL), false);
            // DriverStation.reportError( "ADIS16470 NULL_CNFG register: " +
            // readRegister(NULL_CNFG), false);
            // DriverStation.reportError( "ADIS16470 DEC_RATE register: " +
            // readRegister(DEC_RATE), false);
            // DriverStation.reportError( "ADIS16470 FILT_CTRL register: " +
            // readRegister(FILT_CTRL), false);
            ...
            // DriverStation.reportError( "ADIS16470 Selftest DIAG_STAT register: " +
            // readRegister(DIAG_STAT), false);
    ...
    public int configCalTime(CalibrationTime new_cal_time) {
        if (m_calibration_time == new_cal_time.m_value) {
            return 1;
        }
        if (!switchToStandardSPI()) {
        ...
        m_calibration_time = new_cal_time.m_value;
